...
class Louvain() extends Serializable{
  ...
  def saveLevel(
    ...
    println("vertexSavePath:", vertexSavePath)
    graph.vertices.foreach(println(_))
    // save
    graph.vertices.repartition(1).saveAsTextFile(vertexSavePath)
    graph.edges.repartition(1).saveAsTextFile(edgeSavePath)

    // overwrite the q values at each level
    sc.parallelize(qValues, 1).repartition(1).saveAsTextFile(config.outputDir + "/qvalues_" + level)
  }
