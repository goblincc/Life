  ...
  def main(args: Array[String]): Unit = {
    ...
    val sqlTxt =
      s"""
         ...
         |from (
         |select * from persona.yylive_uid_feature_info WHERE dt='${dt}') as a
         |inner join (
         |select userid  FROM persona.ybpay_paygate_order_d WHERE dt = '${dts}' group by userid) as b
         |on a.uid = b.userid
...
//    piperisk_20210814
    val model = PipelineModel.read.load( modelPath + "/piperisk_20211129")
